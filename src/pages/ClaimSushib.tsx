...
import { SUSHIB_AUTH_MESSAGE, SERVER_URL, SUSHIB } from "../constants";
...
const ClaimSushib = ({ context }: { context: EthereumContext }) => {
  ...
  const onClaim = async () => {
    ...
    if (context.signer) {
      try {
        const signature = await context.signer.signMessage(SUSHIB_AUTH_MESSAGE);
  ...
  return (
    <header className="App-header" style={{ backgroundColor: "midnightblue" }}>
